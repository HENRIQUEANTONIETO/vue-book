<template>
  <div class="container">
    ...
    <div class="flip-book" id="demoBookExample">
      ...
      <div class="page" v-for="(place, index) in filteredPlaces">
        <div class="page-content">
          <h2 class="page-header">{{ place.title }}</h2>
...
const props = defineProps<{ title: string; places: Place[] }>();
...
function searchPlace() {
  const placesFilter = props.places.filter((place: Place) =>
    place.title.toLowerCase().includes(search.value.toLowerCase())
  );

  console.log(placesFilter);

  filteredPlaces.value = placesFilter;
